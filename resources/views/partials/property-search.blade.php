...
<form
  action="{{ $archiveUrl }}"
  method="GET"
  class="sv-search-bar {{ $compact ? 'sv-search-bar--compact' : 'sv-search-bar--sandwich' }}"
  role="search"
  aria-label="{{ __('Search properties', 'sage') }}"
>
  {{-- Sandwich: row 1 = keyword + button, row 2 = filters --}}
  ...
  <div class="sv-search-bar__row sv-search-bar__row--filters">
    ...
    @if(!$compact)
      <div class="sv-search-field">
        <label for="sv-search-price" class="sv-search-field__label">{{ __('Max price', 'sage') }}</label>
        <select id="sv-search-price" name="max_price">
          <option value="">{{ __('Any price', 'sage') }}</option>
          <option value="50000" {{ (($_GET['max_price'] ?? '') === '50000') ? 'selected' : '' }}>≤ $50k</option>
          <option value="100000" {{ (($_GET['max_price'] ?? '') === '100000') ? 'selected' : '' }}>≤ $100k</option>
          <option value="150000" {{ (($_GET['max_price'] ?? '') === '150000') ? 'selected' : '' }}>≤ $150k</option>
          <option value="250000" {{ (($_GET['max_price'] ?? '') === '250000') ? 'selected' : '' }}>≤ $250k</option>
          <option value="500000" {{ (($_GET['max_price'] ?? '') === '500000') ? 'selected' : '' }}>≤ $500k</option>
        </select>
      </div>
    @endif
  </div>
